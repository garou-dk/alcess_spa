<template>
    <div>
        ...
        <div v-if="!loadService.request.loading" id="walkin-sales-report-data">
            <!-- Header -->
            <div
                style="
                    display: flex;
            ...
            <!-- Table -->
            <h2 style="margin-top: 30px; font-size: 16px">Walk-In Sales Overview</h2>
            <table
                ...
                <tbody>
                    <tr v-for="(val, index) in data" :key="index">
                        <td style="border: 1px solid #ccc; padding: 8px">
                            {{ DateUtil.formatToMonthDayYear(val.transaction_date) }}
                        </td>
                        <td style="border: 1px solid #ccc; padding: 8px">
                            {{ val.transaction_id }}
                        </td>
                        ...
                        <td style="border: 1px solid #ccc; padding: 8px">
                            {{ CurrencyUtil.formatCurrency(val.unit_price) }}
                        </td>
                        <td style="border: 1px solid #ccc; padding: 8px">
...
const data = ref<IWalkInSalesData[]>([]);

const printReport = () => {
    const printContents = document.getElementById("walkin-sales-report-data")?.innerHTML;
    ...
    if (printContents && printWindow) {
      ...
      <html>
        <head>
          <title>Walk-In Sales Report</title>
          <style>
            @media print {
...
const loadService = useAxiosUtil<IForm, IWalkInSalesData[]>();
const toast = useToast();

const load = async () => {
    await loadService.get('admin/reports/walkin-sales', form).then(() => {
